...
export async function generateViewData(
  {
    ...
    format,
    query2
  }: Pick<TView, 'name' | 'type' | 'format' | 'query2'> & { id?: string },
  ...
  options.logger &&
    NotionLogger.method.info(`CREATE collection_view ${view_data.id}`);
  return [
    view_data,
    NotionOperations.Chunk.collection_view.update(
      view_id,
      [],
      JSON.parse(JSON.stringify(view_data))
    )
  ] as const;
}
