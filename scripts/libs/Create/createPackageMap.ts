...
export async function createPackageMap(resume?: boolean){
  resume = resume ?? false;
  const package_map_json_data_path = path.resolve(__dirname, '../../../data/package_map.json');
  if(!resume){
    ...
    await fs.promises.writeFile(package_map_json_data_path, JSON.stringify(packages_map, jsonReplacer, 2), 'utf-8');
    return packages_map;
  }else
    return JSON.parse(await fs.promises.readFile(package_map_json_data_path, 'utf-8'), jsonReviver) as IPackageMap;
}