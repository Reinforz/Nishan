...
export type TViewType =
  ...
  | 'calendar'
  | 'timeline';
export type TViewFormatCover =
  | { type: 'page_content' | 'page_cover' }
...
export interface ITableView extends NotionNode, ParentProps, SpaceProps {
  name: string;
  type: 'table';
  page_sort: string[];
...
export interface IListView extends NotionNode, ParentProps, SpaceProps {
  name: string;
  type: 'list';
...
export interface IBoardViewFormat {
  ...
  board_groups2: {
    hidden: boolean;
    property: string;
    value: { type: 'select' | 'multi_select'; value?: string };
  }[];
...
export interface IBoardView extends NotionNode, ParentProps, SpaceProps {
  type: 'board';
  name: string;
...
export interface IGalleryView extends NotionNode, ParentProps, SpaceProps {
  type: 'gallery';
  name: string;
...
export interface ICalendarView extends NotionNode, ParentProps, SpaceProps {
  type: 'calendar';
  name: string;
...
export interface ITimelineView extends NotionNode, ParentProps, SpaceProps {
  type: 'timeline';
  name: string;
...
export type TTimelineViewZoomLevel =
  | 'hours'
  | 'day'
  | 'week'
  | 'bi_week'
