...
export const restoreComments = async (ids: string[], options: INotionCacheOptions & INotionOperationOptions) => {
	...
	for (let index = 0; index < ids.length; index++) {
		...
		comment_data.alive = true;
		discussion_data.comments.push(comment_data.id);
		operations.push(
			NotionOperations.Chunk.comment.set(comment_data.id, [ 'alive' ], true),
			NotionOperations.Chunk.discussion.listAfter(discussion_data.id, [ 'comments' ], {
				id: comment_data.id
			}),
