...
it(`parent=space,isBookmarked=true`, async () => {
  ...
  const cache = {
      ...
      space_view: new Map([
        ['space_view_1', { space_id: 'space_1', id: 'space_view_1' } as any]
      ])
    },
    { e1, executeOperationsMock } = createExecuteOperationsMock();

  const logger_spy = jest
...
[`collection_view_page`, `collection_view`].forEach((collection_block_type) => {
  it(`type=${collection_block_type}`, async () => {
    ...
    expect(logger_spy).toHaveBeenNthCalledWith(
      4,
      `CREATE block ${collection_view.id}`
    );
...
it(`type=embed`, async () => {
  ...
  const cache = {
    ...
    getGenericEmbedBlockDataMock = jest
      .spyOn(NotionEndpoints.Queries, 'getGenericEmbedBlockData')
      .mockImplementation(
        async () =>
          ({
            format: {
