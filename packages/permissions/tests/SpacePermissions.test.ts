...
it(`addMembers`, async () => {
  ...
  const notion_users = await space.addMembers([
    ['user_root_2@gmail.com', 'editor']
  ]);

  expect(findUser).toHaveBeenCalledTimes(1);
  expect(findUser).toHaveBeenNthCalledWith(
    1,
    { email: 'user_root_2@gmail.com' },
  ...
  await expect(() =>
    space.addMembers([['user_root_2@gmail.com', 'editor']])
  ).rejects.toThrow(colors.red.bold(`User does not have a notion account`));
});
...
it(`removeUsers`, async () => {
  ...
  const removeUsersFromSpaceMock = jest
    .spyOn(NotionEndpoints.Mutations, 'removeUsersFromSpace')
    .mockImplementationOnce(() => {
      return {} as any;
    });
