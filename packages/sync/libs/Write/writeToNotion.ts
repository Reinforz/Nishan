...
export async function writeToNotion(
  result_data: INotionSyncFileShape,
  options: INotionOperationOptions
) {
  const metadata = {
      alive: true,
  ...
  const collection_create_block_op = NotionOperations.Chunk.block.update(
    collection_block_id,
    [],
    {
  ...
  const collection_create_op = NotionOperations.Chunk.collection.update(
    collection_id,
    [],
    {
      id: collection_id,
  ...
  const space_after_op = NotionOperations.Chunk.space.listAfter(
    options.space_id,
    ['pages'],
    {
      after: '',
