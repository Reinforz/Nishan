...
describe('constructAndSyncRecordsParams', () => {
  it(`sync_record_values is not empty`, async () => {
    ...
    expect(saveToCacheMock.mock.calls[0][3]).toStrictEqual('space_1');
  });

  it(`sync_record_values is not empty and recordMapWithRoles['space'] doesn't exist`, async () => {
    const cache = NotionCache.createDefaultCache(),
      sync_record_values_response = {
        recordMapWithRoles: {}
      } as any;
    ...
    expect(saveToCacheMock.mock.calls[0][3]).toStrictEqual('');
  });
