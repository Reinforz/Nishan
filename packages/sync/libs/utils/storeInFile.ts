...
export async function storeInFile (filepath: string, result_data: LocalFileStructure) {
	...
	if (ext === '.json')
		await fs.promises.writeFile(filepath, JSON.stringify(ExtractData.extract(result_data), null, 2), 'utf-8');
	else if (ext === '.yaml' || ext === '.yml')
		await fs.promises.writeFile(filepath, dump(ExtractData.extract(result_data)), 'utf-8');
