...
describe('createDiscussions', () => {
  ...
  it(`default input`, async () => {
    ...
    expect(cache.comment.get(comment_1_id)).toStrictEqual(
      expect.objectContaining(comment_1_data)
    );
    expect(cache.discussion.get(discussion_id)).toStrictEqual(
      expect.objectContaining(discussion_data)
    );
    expect(cache.block.get(block_1_id)).toStrictEqual(
      expect.objectContaining({ discussions: [discussion_id] })
    );
  ...
  it(`custom input`, async () => {
    ...
    expect(discussions).toStrictEqual([
      { ...discussion_data, context: [['Different context']] }
    ]);
  });
});
