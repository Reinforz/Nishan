...
class NotionUser extends Data<INotionUser, INotionUserUpdateInput> {
   ...
   */
  getUserSettings() {
    const user_settings = this.cache.user_settings.get(
      this.user_id
    ) as IUserSettings;
    this.logger &&
      NotionLogger.method.info(`READ user_settings ${user_settings.id}`);
    ...
    for (let index = 0; index < opts.length; index++) {
      const opt = opts[index],
        ...
        space_view_id = NotionIdz.Generate.id(),
        {
          spaceId: space_id,
          recordMap: { space }
        } = await NotionEndpoints.Mutations.createSpace(
      ...
      this.cache.space.set(
        space_id,
        JSON.parse(JSON.stringify(space[space_id].value))
      );
      ...
      await NotionOperations.executeOperations(
        [
          ...
          NotionOperations.Chunk.space_view.update(
            space_view_id,
            [],
            JSON.parse(JSON.stringify(space_view_data))
          ),
          NotionOperations.Chunk.user_root.listAfter(
            this.user_id,
            ['space_views'],
            { after: '', id: space_view_id }
          )
        ],
        this.getProps()
      );
      ...
      this.logger &&
        NotionLogger.method.info(`CREATE space_view ${space_view_id}`);
      this.logger &&
        NotionLogger.method.info(`UPDATE user_root ${this.user_id}`);
  ...
  async updateSpaces(
    args: UpdateTypes<ISpace, ISpaceUpdateInput>,
    multiple?: boolean
  ) {
    return await this.updateIterate<ISpace, ISpaceUpdateInput, Space[]>(
      ...
      (id, _, __, spaces) =>
        spaces.push(new Space({ ...this.getProps(), id, space_id: id }))
    );
  }
