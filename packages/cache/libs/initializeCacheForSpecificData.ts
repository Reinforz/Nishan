  ...
  if (should_initialize_cache) {
    ...
    switch (type) {
      case 'block': {
        ...
        if (data.type.match(/^(page|collection_view_page|collection_view)$/)) {
          ...
          while (has_more_chunk) {
            const {
              recordMap,
              cursor
            } = await NotionEndpoints.Queries.loadPageChunk(
              {
                pageId: id,
                limit: 100,
                cursor: {
        ...
        NotionCache.extractSpaceAndParentId(data).forEach((sync_record_value) =>
          container.push(sync_record_value)
        );
        break;
      }
      case 'space': {
      ...
      case 'user_root': {
        ...
        data.space_views.map((space_view) =>
          container.push([space_view, 'space_view'])
        );
        break;
      }
      case 'collection': {
      ...
      case 'comment': {
        ...
        NotionCache.extractNotionUserIds(data).forEach((id) =>
          container.push([id, 'notion_user'])
        );
        container.push([data.space_id, 'space']);
