...
it('create space', async () => {
  ...
  const methodLoggerMock = jest
    .spyOn(NotionLogger.method, 'info')
  ...
  const createSpaceMock = jest
    .spyOn(NotionEndpoints.Mutations, 'createSpace')
    .mockImplementationOnce(async () => {
      return {
        spaceId,
        recordMap: { space: { [spaceId]: { value: space_snapshot_data } } }
      } as any;
    });
  ...
  expect(methodLoggerMock).toHaveBeenNthCalledWith(
    1,
    `CREATE space ${spaceId}`
  );
  expect(methodLoggerMock).toHaveBeenNthCalledWith(
    2,
    `CREATE space_view ${space_views[0]}`
  );
  expect(methodLoggerMock).toHaveBeenNthCalledWith(
    3,
    `UPDATE user_root user_root_1`
  );
  expect(methodLoggerMock).toHaveBeenNthCalledWith(
    4,
  ...
  expect(cache.space_view.get(space_views[0])).toStrictEqual(
    space_view_snapshot_data
  );
  expect(space.getCachedData()).toStrictEqual(
    expect.objectContaining(space_snapshot_data)
  );
  e1([
...
it(`get space`, async () => {
  ...
  expect(space.getCachedData()).toStrictEqual({ id: 'space_2' });
...
it(`update space`, async () => {
  const cache: INotionCache = {
      ...NotionCache.createDefaultCache(),
      space: new Map([
        ['space_1', { id: 'space_1', name: 'Space 1' }],
        ['space_2', { id: 'space_2', name: 'Space 2' }]
      ] as any)
    },
  ...
  const space = await notion_user.updateSpace([
    'space_1',
    { name: 'Space One' }
  ]);
...
it('getUserSettings', () => {
  const cache: INotionCache = {
    ...
    user_settings: new Map([
      ['user_root_1', { id: 'user_root_1', name: 'User Settings 1' }]
    ] as any)
  };
  ...
  expect(user_settings.getCachedData()).toStrictEqual({
    id: 'user_root_1',
...
it('getUserRoot', () => {
  const cache: INotionCache = {
    ...NotionCache.createDefaultCache(),
    user_root: new Map([
      ['user_root_1', { id: 'user_root_1', name: 'User Root 1' }]
    ] as any)
  };
  ...
  expect(user_root.getCachedData()).toStrictEqual({
    id: 'user_root_1',
    name: 'User Root 1'
  });
...
it(`delete space`, async () => {
  const space_1: any = { id: 'space_1', name: 'Space One' },
    cache: INotionCache = {
      ...NotionCache.createDefaultCache(),
