...
it(`updateCacheManually`, async () => {
  const update_cache_manually_args: SyncRecordValuesTuple = [
  ...
  expect(constructSyncRecordsParamsMock.mock.calls[0][0]).toStrictEqual(
    update_cache_manually_args
  );
  expect(constructSyncRecordsParamsMock.mock.calls[0][1]).toStrictEqual(
