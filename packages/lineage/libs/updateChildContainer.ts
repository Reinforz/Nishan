...
export async function updateChildContainer<T extends TData>(
  ...
  const parent_data = await NotionCache.fetchDataOrReturnCached(
    parent_table,
    parent_id,
    options
  );
  ...
  if (!keep && container.includes(child_id)) {
    (parent_data as any)[child_path] = container.filter(
      (page_id) => page_id !== child_id
    ) as any;
    operations.push(
      NotionOperations.Chunk[parent_table].listRemove(
        parent_data.id,
        space_id,
        [child_path as string],
        {
          id: child_id
        }
      )
    ...
    operations.push(
      NotionOperations.Chunk[parent_table].listAfter(
        parent_data.id,
        space_id,
        [child_path as string],
        {
