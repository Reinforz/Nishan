...
import { getSchemaUnit, InitializeView, ISchemaAggregationMap } from "../";
...
export function aggregations(data: ITableView | IBoardView | ITimelineView, schema: Schema){
  ...
  aggregations.forEach((aggregation, index) => {
    const schema_unit = getSchemaUnit(schema, aggregation.property, ["query2", "aggregation", index.toString()] );
