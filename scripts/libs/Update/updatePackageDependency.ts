...
export async function updatePackageDependency (package_name: string, packages_map: IPackageMap) {
	...
	[ 'dependencies', 'devDependencies' ].forEach((dependency_type) => {
        ...
        package_json_data[dependency_type][dependency_name] = packages_deps_info.version;
        console.log(colors.bold.blue(`${dependency_type} ${dependency_name} ${packages_deps_info.version}`));
      }
		});
	});
