...
describe('getPagesById', () => {
  it('Work correctly for type=page & cvp', async () => {
    ...
    const notion = new NotionCore.Api.Nishan({
      ...
      cache: {
        ...
        collection_view: new Map([
          ['collection_view_1', { id: 'collection_view_1' } as any]
        ])
      } as any
    });
    ...
    expect(
      initializeCacheForSpecificDataMock.mock.calls[0].slice(0, 2)
    ).toEqual([block_1_id, 'block']);
    expect(
      initializeCacheForSpecificDataMock.mock.calls[1].slice(0, 2)
    ).toEqual([block_2_id, 'block']);

    expect((page_map.page.get(block_1_id) as Page).id).toStrictEqual(
      block_1_id
    );
    ...
    expect(
      (page_map.collection_view_page.get(block_2_id) as CollectionViewPage).id
    ).toStrictEqual(block_2_id);
    const collection_view_page_obj = page_map.collection_view_page.get(
      'Collection One'
    ) as CollectionViewPage;
